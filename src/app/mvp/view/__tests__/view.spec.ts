...
const recievedCoords = { from: 2001, to: 6000 };
...
describe('View', () => {
  ...
  afterEach(() => {
    view.updateViewOptions(defaultOptions, recievedCoords);
    view.render();
  });
  ...
  it('checks option "double" to remove or add a hiding class to first dot', () => {
    view.updateViewOptions({ ...defaultOptions, double: false }, recievedCoords);
    view.render();
    const { firstDot } = getSliderElems();
    expect(firstDot.classList.contains('slider_hidden')).toBeTruthy();

    view.updateViewOptions({ ...defaultOptions, double: true }, recievedCoords);
    view.render();
    expect(firstDot.classList.contains('slider_hidden')).toBeFalsy();
  });

  it('adds verical class if option "vertical" is true', () => {
    view.updateViewOptions({ ...defaultOptions, vertical: true }, recievedCoords);
    view.render();
  ...
  it('values change on scale click', () => {
    view.updateViewOptions({ ...defaultOptions, scale: true }, recievedCoords);
    view.render();
    const mock = jest.fn();
  ...
  describe('sets the right styles for dots', () => {
    ...
    describe('orientaion', () => {
      ...
      it('vertical', () => {
        view.updateViewOptions({ ...defaultOptions, double: true, vertical: true }, recievedCoords);
        view.render();
  ...
  describe('sets the right styles for filler', () => {
    ...
    describe('orientaion', () => {
      it('horizontal', () => {
        view.updateViewOptions({ ...defaultOptions, from: 0 }, recievedCoords);
        view.render();
